...
class PricingAgent:
    ...
    def analyze_product(self, product_text):
        ...
        # Average across suppliers/customers
        avg_supplier_momentum = np.mean([s. get('momentum_3d_pct', 0) for s in supplier_signals])
        avg_supplier_pct30 = np.mean([s. get('pct_30d', 0) for s in supplier_signals])
        avg_supplier_spike = max([s.get('volume_spike', 0) for s in supplier_signals])
        
        avg_customer_momentum = np.mean([c.get('momentum_3d_pct', 0) for c in customer_signals])
        avg_customer_pct30 = np.mean([c.get('pct_30d', 0) for c in customer_signals])
        avg_customer_spike = max([c. get('volume_spike', 0) for c in customer_signals])
        ...
        # Map profit prediction to price recommendation
        # Simple mapping: if predicted profit is high with certain price change, recommend that change
        # For now:  if profit > 0, suggest +5%, else hold
        if pred_profit[0] > 2: 
            price_action = 5
        ...
        result = {
            ...
            'suppliers': [{'name': s['name'], 'ticker': s['ticker'], 'pct_30d': s. get('pct_30d')} for s in supplier_signals],
            'customers': [{'name': c['name'], 'ticker': c['ticker'], 'pct_30d': c.get('pct_30d')} for c in customer_signals],
        }
